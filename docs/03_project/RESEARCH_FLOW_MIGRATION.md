...
0. 现有完整行业研究是所有业务场景共享的 Research Core，不是可被新场景替代或复制的页面流程。PE、VC 和企业增长只增加决策目标、访谈、场景工作流、人工 Gate 和交付物；行业定义、产业链、独立市场规模测算、竞争、驱动、Future Intelligence、证据治理与报告修订必须复用同一基座。
...
7. 场景扩展采用注册式 Scenario Pack；应用主流程不得通过大量 `if pe`、`if vc` 或 `if growth_strategy` 编排业务。
