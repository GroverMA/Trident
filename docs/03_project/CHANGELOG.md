...
- 场景原生的动态诊断访谈持久化：企业增长、PE、VC 由各自 Scenario Pack 声明诊断主题和问题，前端创建真实项目并逐轮保存回答。
- 结构化企业/标的画像工件，保存经营画像、决策风格、研究下一步、数据缺口与原始访谈轮次引用。
- 独立的场景选择、项目管理、企业知识库、持续感知、决策与行动质量工作台，以及跨页面项目联动上下文。
- Growth、PE、VC 的行动反馈、候选计划调整和知识库写回编排合同。
...
- DeepSeek 结构化研究任务启用 JSON Output、至少 8,000 Token 输出预算和一次非法 JSON 修复重试，避免 Research Plan 在模型已成功响应后因格式或截断失败。
...
- 当前动态追问采用确定性的场景策略编排；模型生成追问、画像人工确认与知识库正式写回将在后续增量完成。
