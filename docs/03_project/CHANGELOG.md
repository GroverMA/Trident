...
- 研究计划首次返回空任务或结构不合格时，第二轮改由深度模型结合具体违规原因修复；任务数量保持自适应，但空计划始终禁止。
- 审阅式与构建式研究工作台为全部等待动作统一显示当前阶段、预计耗时范围和实时已等待时间。
- 研究计划不再把早期验收用的 6–10 个任务误当成专业 SOP 硬限制；任务数改由问题复杂度和证据边界决定，同时继续强制模块覆盖、用户问题覆盖、证据标准与反证要求。
