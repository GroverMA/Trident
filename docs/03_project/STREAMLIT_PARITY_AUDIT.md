...
|---|---|---|---|
| 项目创建与研究方式 | 通用/企业，构建式/审阅式，共享项目 | 模式前置入口及创建首页已按原版恢复 | 后续补齐案例加载与企业字段联动验收 |
| 左侧项目管理 | 搜索、进行中/历史、当前项目、项目操作、文件夹分类、工作台与流程控制 | 共享侧栏壳层已恢复；搜索、列表、当前项目和工作台入口可用 | 补齐终止/删除、文件夹持久化和 Rewind API |
| Prompt Analysis | AI 先解释原始 Prompt | 已纠正并迁移 | 保持 AI 先于人工范围确认 |
| Gate 0 可编辑范围 | 编辑核心市场、产品、客户、地域、时间、产业链、纳入/排除、相邻市场、规模与竞争口径、歧义和回答 | 已按原版完整字段与确认动作重做 | 部署后做真实模型和持久化验收 |
...
- `/` 首屏只呈现 `Research Build First` 与 `Report Review First`，桌面与移动端一致；未选择前不渲染项目首页。
- 选择研究方式后才进入项目首页，左侧为项目管理，主区域为创建研究和产品说明。
- 构建式工作台固定展示八节点：`Prompt Analysis → Gate 0 · Scope → Web Research → Gate 1 · Evidence → Industry Analysis → Future Intelligence → Gate 2 · Content → General Report`。
- Prompt Analysis 只做语义理解，不进行网页搜索；Gate 0 确认后才允许进入网页研究。
- 视觉验收记录位于仓库根目录 `design-qa.md`。
