...
function EnterpriseFlow({ growthType }: { growthType: string }) {
  return <section className="enterpriseFlow" id="memory"><div className="enterpriseFlowHead"><div><span className="eyebrow">ENTERPRISE DECISION WORKFLOW</span><h2>{growthType}决策流</h2></div><p>行业研究只围绕增长决策所需的问题展开，不生成通用行业报告。</p></div><div className="enterpriseFlowGrid">
    ...
    <article><span>02</span><h3>市场机会与下游应用</h3><p>围绕目标建立候选机会，重点判断客户需求、应用价值、进入条件和竞争风险。</p><small>预留下游应用筛选 Skill 接口</small></article>
    <article><span>03</span><h3>Company Scorecard</h3><p>把市场吸引力与企业能力、资源、执行风险进行匹配，收敛至最多三条路径。</p><small>机会优先级 · 能力差距 · 资源配置</small></article>
    <article><span>04</span><h3>Action Plan 与增长复盘</h3><p>形成分阶段行动、负责人、预算、领先指标和复盘触发条件。</p><small>反馈 → 更新 → 优化 → 应用</small></article>
  </div><div className="enterpriseOutputs"><div><strong>Company Scorecard</strong><span>市场吸引力 / 企业适配度 / 可执行性 / 风险 / 资源缺口</span></div><div><strong>Action Plan</strong><span>90 天动作 / 负责人 / 预算 / KPI / 停止或调整条件</span></div></div></section>;
}
