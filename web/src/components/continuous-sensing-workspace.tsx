...
export function ContinuousSensingWorkspace() {
  ...
  return <main>
    ...
    <section className="platformPage">
      ...
      <div className="sensingSubscription"><div><span className="eyebrow">WATCH TERMS</span><h2>公司、行业与一手来源</h2><p>关注词用于发现变化；可登记已获授权的公司官网、监管、交易所或专业媒体 RSS。一手来源优先进入审核队列。</p></div><label><span>关注词（逗号分隔）</span><input value={watchTerms} onChange={(event) => setDrafts((current) => ({ ...current, [activeId]: event.target.value }))} placeholder="公司、行业、竞争者、技术主题"/></label><label><span>自动感知频率</span><select value={artifact?.subscription?.enabled ? artifact.subscription.cadence : "manual"} disabled={!active || busy === "subscription"} onChange={(event) => void saveSubscription(event.target.value as "manual" | "daily" | "weekly")}><option value="manual">仅手动刷新</option><option value="daily">每日</option><option value="weekly">每周</option></select></label><label><span>新增来源类型</span><select value={sourceType} onChange={(event) => setSourceType(event.target.value as typeof sourceType)}><option value="company_official">公司官方</option><option value="regulator_government">政府/监管</option><option value="exchange_disclosure">交易所披露</option><option value="professional_media">专业媒体</option></select></label><label><span>RSS 地址（HTTPS）</span><input value={sourceUrl} onChange={(event) => setSourceUrl(event.target.value)} placeholder="https://example.com/feed.xml"/></label><button className="primaryButton" disabled={!active || busy === "refresh"} onClick={() => void refresh()}>{busy === "refresh" ? "正在抓取并去重…" : sourceUrl ? "保存来源并刷新" : "立即刷新新闻"}</button>{artifact?.subscription?.next_run_at && <p className="sensingSchedule">下次自动运行：{new Date(artifact.subscription.next_run_at).toLocaleString("zh-CN")}</p>}{artifact?.sources?.length ? <div className="sensingSources">{artifact.sources.map((source) => <span key={source.source_id} className={`source-${source.status}`}>{source.name} · Tier {source.tier} · {source.status === "succeeded" ? "连接成功" : source.status === "failed" ? "连接失败" : "待检查"}</span>)}</div> : null}{error && <p className="formError">{error}</p>}{artifact?.fetch_errors?.length ? <p className="sensingWarning">部分来源暂不可用：{artifact.fetch_errors.join("；")}。历史信号和其他来源不受影响。</p> : null}</div>
