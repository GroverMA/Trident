...
const policyStages: Record<string, string> = {
  draft: "征求意见/草案", issued: "已发布", effective: "已生效",
  amended: "已修订", repealed: "已废止", unknown: "阶段待核验",
};
...
export function ContinuousSensingWorkspace() {
  ...
  const policies = artifact?.policy_records || [];
  const policyById = new Map(policies.map((item) => [item.policy_id, item]));
  ...
  return <main>
    ...
    <section className="platformPage">
      ...
      {policies.length > 0 && <details className="policyRegistry" open><summary>政策与监管版本库 · {policies.length} 项</summary><div className="policyRegistryGrid">{policies.slice(0, 12).map((policy) => { const latest = policy.versions[0]; return <article key={policy.policy_id}><header><span>{policyStages[policy.current_stage]}</span><small>{policy.versions.length} 个版本</small></header><h3>{policy.canonical_title}</h3><dl><div><dt>发布机关</dt><dd>{policy.issuing_authority}</dd></div><div><dt>适用地区</dt><dd>{policy.jurisdiction || "待核验"}</dd></div><div><dt>文号</dt><dd>{policy.document_number || "待核验"}</dd></div><div><dt>生效日期</dt><dd>{latest.effective_date ? new Date(latest.effective_date).toLocaleDateString("zh-CN") : "待核验"}</dd></div></dl><p>适用对象：{policy.applicability.join("、") || "待人工补充"}</p><footer><a href={latest.source_url} target="_blank" rel="noreferrer">查看当前来源</a><small>版本 {latest.version_id}</small></footer>{policy.versions.length > 1 && <details><summary>查看历史版本</summary><ul>{policy.versions.slice(1).map((version) => <li key={version.version_id}><a href={version.source_url} target="_blank" rel="noreferrer">{version.title}</a><small>{version.publication_date ? new Date(version.publication_date).toLocaleDateString("zh-CN") : "日期待核验"} · {policyStages[version.lifecycle_stage]}</small></li>)}</ul></details>}</article>; })}</div><p className="policyNotice">自动抽取字段均为待审核元数据；正式引用仍需打开原始文件核对发布机关、文号、生效条款和适用范围。</p></details>}
