...
export function ContinuousSensingWorkspace() {
    ...
    try {
      const response = await fetch(`/api/projects/${active.project_id}/continuous-sensing/internal-kpi/feishu/sync`, {
        ...
        body: JSON.stringify({ connector: {
          ...
          view_id: feishuViewId.trim() || null,
        }}),
      });
      const payload = await response.json();
  ...
  return <main>
    ...
    <section className="platformPage">
      ...
      <details className="kpiBatchImport feishuConnector"><summary>连接飞书多维表格</summary><div><p>应用凭证只配置在 Trident 服务端；这里不填写密钥。多维表格需包含“指标名称、本期数值、单位、期间”列，可选“判断方向、上期值、目标值、经营说明”。</p><label><span>连接名称</span><input value={feishuName} onChange={(event) => setFeishuName(event.target.value)}/></label><label><span>App Token</span><input value={feishuAppToken} onChange={(event) => setFeishuAppToken(event.target.value)} placeholder="多维表格 URL 中的 app..."/></label><label><span>Table ID</span><input value={feishuTableId} onChange={(event) => setFeishuTableId(event.target.value)} placeholder="tbl..."/></label><label><span>View ID（可选）</span><input value={feishuViewId} onChange={(event) => setFeishuViewId(event.target.value)} placeholder="vew..."/></label><button className="primaryButton" disabled={!active || !feishuAppToken.trim() || !feishuTableId.trim() || busy === "feishu-kpi"} onClick={() => void syncFeishuKpis()}>{busy === "feishu-kpi" ? "正在读取并形成信号…" : "测试连接并同步"}</button>{artifact?.kpi_connectors?.map((connector) => <div className="connectorStatus" key={connector.connector_id}><strong>{connector.name}</strong><span>{connector.status === "succeeded" ? `已同步 ${connector.last_record_count} 条` : "待同步"}</span><small>{connector.last_synced_at ? new Date(connector.last_synced_at).toLocaleString("zh-CN") : "尚未运行"}</small></div>)}</div></details>
