...
export function ContinuousSensingWorkspace() {
  ...
  return <main>
    ...
    <section className="platformPage">
      ...
      <div className="sensingSubscription"><div><span className="eyebrow">WATCH TERMS</span><h2>公司与行业关注词</h2><p>默认来自项目公司、行业、地区和市场范围；可补充竞争者、技术、客户或政策主题。</p></div><label><span>关注词（逗号分隔）</span><input value={watchTerms} onChange={(event) => setDrafts((current) => ({ ...current, [activeId]: event.target.value }))} placeholder="公司、行业、竞争者、技术主题"/></label><label><span>自动感知频率</span><select value={artifact?.subscription?.enabled ? artifact.subscription.cadence : "manual"} disabled={!active || busy === "subscription"} onChange={(event) => void saveSubscription(event.target.value as "manual" | "daily" | "weekly")}><option value="manual">仅手动刷新</option><option value="daily">每日</option><option value="weekly">每周</option></select></label><button className="primaryButton" disabled={!active || busy === "refresh"} onClick={() => void refresh()}>{busy === "refresh" ? "正在抓取并去重…" : "立即刷新新闻"}</button>{artifact?.subscription?.next_run_at && <p className="sensingSchedule">下次自动运行：{new Date(artifact.subscription.next_run_at).toLocaleString("zh-CN")}</p>}{error && <p className="formError">{error}</p>}{artifact?.fetch_errors?.length ? <p className="sensingWarning">部分来源暂不可用：{artifact.fetch_errors.join("；")}。历史信号和其他来源不受影响。</p> : null}</div>
